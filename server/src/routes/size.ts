...
app.post("/size", async (c) => {
  ...
  const { tripId, name, size, recipientName } = await c.req.json<{
    tripId: unknown;
    name: string;
    size: string;
  ...
  if (typeof tripId !== "number")
    return c.json({ error: "missing or invalid tripId" }, 400);
...
app.post("/notify", async (c) => {
  const service = createSizeService(c.get("db"), c.get("sendEmail"));
  const { tripId, name, email, recipientName } = await c.req.json<{
    tripId: unknown;
    name: string;
    email: string;
    recipientName: string;
  ...
  return c.json(
    await service.subscribeNotification(tripId, name, email, recipientName),
  );
});
